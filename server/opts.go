...
type WebsocketOpts struct {
	...
	Password string
	Token    string
	// If specified, limit the users/nkeys allowed to connect as websocket clients.
	Users []string
	Nkeys []string
...
type authorization struct {
	...
	// In some cases, users and nkeys are simple string arrays
	nkeysValues []string
	usernames   []string
}
...
func parseAuthForWS(v interface{}, errors *[]error, warnings *[]error) *authorization {
	...
	for mk, mv := range am {
		...
		case "timeout":
			...
			auth.timeout = at
		case "users":
			auth.usernames = parseStringArray("users", tk, &lt, mv, errors, warnings)
		case "nkeys":
			auth.nkeysValues = parseStringArray("nkeys", tk, &lt, mv, errors, warnings)
		default:
			if !tk.IsUsedVariable() {
				err := &unknownConfigFieldErr{
					field: mk,
...
func parseStringArray(fieldName string, tk token, lt *token, mv interface{}, errors *[]error, warnings *[]error) []string {
	switch mv := mv.(type) {
	case string:
		return []string{mv}
	case []interface{}:
...
func parseWebsocket(v interface{}, o *Options, errors *[]error, warnings *[]error) error {
	...
	for mk, mv := range gm {
		...
		case "allowed_origins", "allowed_origin", "allow_origins", "allow_origin", "origins", "origin":
			o.Websocket.AllowedOrigins = parseStringArray("allowed origins", tk, &lt, mv, errors, warnings)
		...
		case "authorization", "authentication":
			...
			o.Websocket.Users = auth.usernames
			o.Websocket.Nkeys = auth.nkeysValues
		case "jwt_cookie":
